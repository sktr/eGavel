...
export function BackupSection() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setError(res.error === "INVALID_RECOVERY_INPUT" ? "Invalid input" : res.error ?? "Restore failed")
    }
  }
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <span className="material-icons" style={{ fontSize: 18, color: "var(--accent)" }}>key</span>
        <h2 style={{ fontSize: 17, fontWeight: 600 }}>Account backup</h2>
      </div>
      <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 16, lineHeight: 1.6 }}>
        These 12 words (or the secret key) are your account. Restore it after
        clearing browser data or on another device by entering the same phrase.
      ...
      {phrase ? (
        <div style={{ marginBottom: 16 }}>
          ...
          >
            {revealed ? phrase : phrase.split(" ").map(() => "••••").join(" ")}
          </div>
      ...
      <form onSubmit={submit} style={{ borderTop: "1px solid var(--border)", paddingTop: 16 }}>
        ...
        <p style={{ fontSize: 11, color: "var(--amber)", marginTop: 8 }}>
          ⚠ Restoring replaces the current key. Active bids and listings are not carried over to the new key.
        </p>
      </form>
    </div>
  )
