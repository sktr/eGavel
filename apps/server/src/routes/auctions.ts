...
import { validateClaim, parseProofData, computeClaimSplit, type StoredProofBundle } from "../claim.js"
...
export function createAuctionRoutes(db: Db) {
  ...
  router.post("/auctions", async (c) => {
    ...
    if (!item || !description || !sellerPubkey || !mintUrl) {
      return c.json({ error: "missing required fields: item, description, seller_pubkey, mint_url" }, 400)
    }
    ...
    const auction: Auction = {
      ...
      reserve_price: typeof body.reserve_price === "number" && body.reserve_price > 0 ? body.reserve_price : null,
      buy_now_price: typeof body.buy_now_price === "number" && body.buy_now_price > 0 ? body.buy_now_price : null,
      end_time: endTime,
      ...
      ...(typeof body.condition === "string" && body.condition ? { condition: body.condition } : {}),
    ...
    try {
      ...
      const inputs = bundle.proofs.map((p) => ({
        ...
        witness: JSON.stringify({
          signatures: [
            seller_sigs[secrets.indexOf(p.secret)]!,
            signSecret(p.secret, serverSkHex),
          ],
        }),
      }))
      ...
      const sellerProofs = sellerOutputs.map((o, i) =>
        o.toProof(swapRes.signatures[i]!, keyset),
      )
      ...
      const feeProofs = feeOutputs.map((o, i) =>
        o.toProof(
          swapRes.signatures[sellerOutputs.length + winnerOutputs.length + i]!,
          keyset,
        ),
      )
      ...
      if (winnerProofs.length > 0 && auction.winner_npub) {
        await db.saveChange(
          ...
          auction.winner_npub,
          change,
          JSON.stringify(winnerProofs),
        )
      }

      return c.json({ seller_proofs: sellerProofs, fee, change })
    } catch (err) {
      return c.json({ error: `claim swap failed: ${err instanceof Error ? err.message : String(err)}` }, 500)
    }
  })
